...
func TestCalculatingCofactorOf3x3Matrix(t *testing.T) {
	...
	assert.Equal(t, 15.0, Minor(a, 1, 1))
	assert.Equal(t, 15.0, Cofactor(a, 1, 1))
}
...
func TestCalculatingDeterminantOf3x3Matrix(t *testing.T) {
	a := Parse(`
		...
		| -5 | 8 | -4|
		|  2 | 6 |  4|`)
	assert.Equal(t, 56.0, Cofactor(a, 0, 0))
	assert.Equal(t, 12.0, Cofactor(a, 0, 1))
	assert.Equal(t, -46.0, Cofactor(a, 0, 2))
	assert.Equal(t, -196.0, Determinant(a))
}
...
func TestCalculatingDeterminantOf4x4Matrix(t *testing.T) {
	a := Parse(`
		...
		|-6| 7| 7|-9|
	`)
	assert.Equal(t, 690.0, Cofactor(a, 0, 0))
	assert.Equal(t, 447.0, Cofactor(a, 0, 1))
	assert.Equal(t, 210.0, Cofactor(a, 0, 2))
	assert.Equal(t, 51.0, Cofactor(a, 0, 3))
	assert.Equal(t, -4071.0, Determinant(a))
}
...
func TestInvertibleMatrix(t *testing.T) {
	a := Parse(`
    	...
    	|  4 | -9 |  3 | -7 |
		|  9 |  1 |  7 | -6 |`)
	assert.Equal(t, -2120.0, Determinant(a))
...
func TestNonInvertibleMatrix(t *testing.T) {
    ...
    |  0 | -5 |  1 | -5 |
    |  0 |  0 |  0 |  0 |`)
	assert.Equal(t, 0.0, Determinant(a))
	assert.False(t, IsInvertible(a))
}
...
func TestCalculatingInverseOfMatrix(t *testing.T) {
	...
	`)
	assert.Equal(t, 532.0, Determinant(a))
	assert.Equal(t, -160.0, Cofactor(a, 2, 3))
	assert.Equal(t, -160.0/532.0, b.At(3, 2))
	assert.Equal(t, 105.0, Cofactor(a, 3, 2))
	assert.Equal(t, 105.0/532.0, b.At(2, 3))
	assert.True(t, Equal(expected, b))
}
...
func TestMultiplyingAProductByItsInverse(t *testing.T) {
	...
	b := `
		...
		|  7 |  0 |  5 |  4 |
		|  6 | -2 |  0 |  5 |`
	c := Multiply(Parse(a), Parse(b))
	assert.True(t, Equal(Parse(a), Multiply(c, Inverse(Parse(b)))))
}
